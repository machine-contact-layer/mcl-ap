...
| Band-informed FSK candidate vs Exp 001 control | E3/E4-partial (Exp 003) | measured, single session | changing only the mark tone 5000 -> 6000 Hz took exact recovery from 0/10 to 9/10 on the DFR1154 receiver and 0/10 to 4/10 on the laptop Realtek receiver, with the 3000/5000 control captured in the same session on the same rig | AP-B0 NOT selected; 3000/6000 is tuned to this measured path and has no general validity; one transmitter, one room, one geometry, one session; 4/10 is not a usable link |
| Reverse-direction multi-device over-air recovery | E4 | RECOVERED 8/10 | DFR1154 MAX98357 speaker transmitting the Experiment 003 candidate to the laptop Realtek microphone array: 10/10 acquisition at correlation 0.883-0.889 and 8/10 exact Wire plus exact PRESENCE recovery. Neither transducer is shared with E3 and the direction is reversed. Evidence under experiments/003-band-informed-candidate/evidence/e4-board-speaker-to-laptop-mic-20260902/ | AP-B0 NOT selected; the 3000/6000 pair was chosen from a forward-path sweep and this reverse path has not been swept; two devices in reversed roles rather than a third independent device; one operator, one room, one session; no range, motion, multipath or multi-node claim |
| Protocol stack running on the embedded peer, over air, both directions | E4 | measured, 4 cells x 10 trials (Exp 004) | The DFR1154 builds a major-1 Tier-0 PRESENCE with mcl-wire, wraps it in a major-1 Link frame with mcl-link, modulates and emits it -- and in the other direction acquires, demodulates, CRC-verifies, decodes the frame and the object, and reports field values, with no host in the loop. board->host 9/10 (10-byte object) and 2/10 (24-byte frame); host->board 6/10 (10-byte) and 7/10 (24-byte), decoded ON the microcontroller. All four cells acquired 10/10, so every failure is a payload bit error rather than a missed preamble. Evidence under experiments/008-embedded-node/evidence/ | NOT a second implementation: the board compiles the same three source files as the host, with a different toolchain for a different architecture, so this is portability and end-to-end operation, not independent interoperability. AP-B0 still NOT selected. 2/10-9/10 is not a usable link. One room, one geometry, one operator, one session. Trailing silence is 0.10 s rather than Experiment 003's 0.5 s, so these numbers are not directly comparable to that evidence |
| Directional asymmetry between the two peers | E4 observation (Exp 004) | measured, same session | board->host recovery falls 9/10 -> 2/10 when the payload grows 2.2x at unchanged acquisition quality; host->board does not (6/10 -> 7/10). The two directions differ in kind, not only in degree, which is what the architecture predicts and had not been measured | one session, one geometry; no model of why, and no general claim about either transducer |
| Consumer audio processing as part of the channel | E2/E4 observation (Exp 004) | uncontrolled, reported as an observation | The Nahimic processing driver was disabled partway through the session; the pilot before it recovered 2/2 at correlation 0.92/0.89 and the full run after it recovered 2/10 at 0.55-0.88. Consumer audio paths are built for human listeners -- enhancement, AGC, noise suppression and beam-forming all assume speech -- and they are present on nearly every laptop and phone | NO controlled pair was run, so this is not a measured effect size and must not be quoted as one. It motivates measuring the path including its driver stack; it does not quantify the driver stack |
| Second receiver on frozen Exp 001 waveform | E4-partial | measured, 10 trials | laptop Realtek array acquired 8/10 at correlation 0.35-0.53 and recovered 0/10, decoding payload_length=11 with a header CRC one to two bits from 0x219A | same transmitter as E3, so this is a second receiver rather than a full second device pair; no E4 completion claim |
...
1b. DONE 2026-09-02. Board-speaker transmit firmware exists (firmware v2, PLAY command) and the reverse direction now recovers 8/10. Transmitter and receiver response are separated: the same laptop microphone recovers 0/10 from the laptop speaker and 8/10 from the board speaker, confirming the notch is in the laptop speaker.
1c. Sweep the reverse path with Experiment 002. The 3000/6000 pair is optimised for the forward path only, and directional asymmetry is expected rather than assumed.
