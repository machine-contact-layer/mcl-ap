...
/* Canonical E3 PRESENCE vector generated by the MCL Wire codec. */
static const uint8_t e3_expected_wire[] = {
...
int main(int argc, char **argv)
{
    ...
    if (est != EXP001_OK) {
        ...
        fprintf(stderr, "  Candidate: start=%zu, correlation=%.6f, preamble_end=%zu\n",
                decode_res.estimated_preamble_start,
                decode_res.peak_correlation,
                decode_res.preamble_end_sample);
        fprintf(stderr, "  Decoded header: payload_length=%zu, received_crc=0x%04X, computed_crc=0x%04X\n",
                decode_res.payload_bytes,
                decode_res.received_crc,
                decode_res.computed_crc);
        if (est == EXP001_ERR_PREAMBLE_NOT_FOUND) {
            fprintf(stderr, "  Preamble not acquired (correlation threshold unmet)\n");
            fprintf(stderr, "  Threshold: %.6f\n", fconfig.detection_threshold);
        }
    ...
    if (decode_res.payload_bytes != sizeof(e3_expected_wire) ||
        memcmp(recovered_bytes, e3_expected_wire, sizeof(e3_expected_wire)) != 0) {
        fprintf(stderr, "E3 exact-vector verification failed\n");
        return 4;
    }
    printf("  Exact E3 Wire vector: MATCH\n");
    ...
    if (wire_consumed != sizeof(e3_expected_wire) ||
    ...
    printf("  Exact E3 semantic object: MATCH\n");
    printf("\n>>> STATUS: E3 BIT-PERFECT RECOVERY VERIFIED <<<\n");
    return 0;
}
